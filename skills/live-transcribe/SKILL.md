...
```bash
PLUGIN_DIR="$(git rev-parse --show-toplevel)/skills/live-transcribe"
ELEVENLABS_API_KEY="$ELEVENLABS_API_KEY" \
  LIVE_TRANSCRIBE_SOUNDS_DIR="$PLUGIN_DIR/assets" \
  nohup python3 "$PLUGIN_DIR/scripts/realtime-transcribe.py" > /tmp/realtime-transcribe.log 2>&1 &
```

The API key must be set in the environment. Check `$PLUGIN_DIR/scripts/.env` or the user's env.
...
Pre-recorded sounds in `skills/live-transcribe/assets/` play automatically:
